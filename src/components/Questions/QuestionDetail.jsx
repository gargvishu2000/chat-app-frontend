...
const QuestionDetail = () => {
  ...
  const handleVote = async (type) => {
    ...
    try {
      const res = await axiosInstance.put(`/questions/${type}/${id}`);
      setQuestion({
        ...question,
    ...
    <div className="max-w-3xl mx-auto p-6 bg-white shadow-lg rounded-lg mt-6">
      ...
      <div className="flex items-center space-x-6 mt-4">
        <button
          className={`flex items-center space-x-1 px-4 py-2 rounded-lg ${
            authUser && question.upvotes.includes(authUser.id)
        ...
        <button
          className={`flex items-center space-x-1 px-4 py-2 rounded-lg ${
            authUser && question.downvotes.includes(authUser.id)
      ...
      {authUser && (
        <div className="mt-6 flex space-x-4">
          {question.author._id === authUser.id ? (
            <>
              <Link
